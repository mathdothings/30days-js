...
function getUniqueLanguuagesWithCount() {
  const uniqueLanguagesWithCount = [];
  ...
  uniqueLanguages.forEach((language) => {
    ...
    uniqueLanguagesWithCount.push({ language: language, count: count });
  });

  return uniqueLanguagesWithCount;
}
// console.log(getUniqueLanguuagesWithCount());

function sortLanguagesAscending() {
  const sorted = ([] = getUniqueLanguuagesWithCount());
...
const uniqueLanguagesWithCount = getUniqueLanguuagesWithCount(uniqueLanguages);
const sortedUniqueLanguagesWithCount = sortLanguagesAscending(
  uniqueLanguagesWithCount
);
// console.log(sortedUniqueLanguagesWithCount);
...
function mostSpokenLanguages(amout) {
  ...
  for (let x = 0; x < amout; x++) {
    let item = sortedUniqueLanguagesWithCount[x];
    lang = item["language"];
    value = item["count"];
    set.add({ [lang]: value });
  }
